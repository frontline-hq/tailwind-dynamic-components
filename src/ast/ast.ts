...
import type { Result } from "../utilities/result";
...
export type RunOn<N extends types.namedTypes.Node | Node, T> = (
    ...
    | ((meta: NodeInfoMap<types.namedTypes.Node | Node | null>) => T)
    | undefined;
...
const findAst: FindAst<any> =
    <W>(walker: typeof jsWalk) =>
    (sourceAst: W, matchers, runOn) => {
        const matchCount = new Map<Node, number>();
        const matches: Node[] = [];
        const nodeInfoMap: NodeInfoMap<Node | types.namedTypes.Node | null> =
            new Map();
        ...
        walker(sourceAst as Node, {
            enter(node, parent, key, index) {
                ...
                const matchCountAncestor = !parent
                    ? 0
                    : matchCount.get(parent) ?? 0;
                if (matchCountAncestor < matchers.length) {
                    const matcher = matchers[matchCountAncestor];
                    const isMatching = matcher
                        ? matcher({ node, parent, key, index })
                        : false;
                    if (isMatching) {
        ...
        for (const match of matches) {
            ...
            let currentNode: Node | types.namedTypes.Node | null | undefined =
                match;
            while (currentNode) {
            ...
            const runOnNode = matchPath.find(
                node => nodeInfoMap.get(node)?.runOnNode
            );
            ...
            if (!runOnNode)
                return [
                    undefined,
                    new Error(
                        "Can't find specified insertion point"
                    ) as FindAstError,
                ];
    ...
    const locations = findAstSvelte(
        ...
        [
            ({ node }) =>
                ...
                reactiveIdentifiers.includes(node.name),
        ],
        node =>
            n.Identifier.check(node) &&
            Object.hasOwn(node, "start") &&
            Object.hasOwn(node, "end")
...
export const getReactiveImportIdentifiers = (importNames: [string, string][]) =>
    importNames.flatMap(([imported, local]) =>
...
export const functionMatchers = (
    name: string
): Parameters<typeof findAstJs>[1] => [
    ({ node }) => n.FunctionDeclaration.check(node),
    ({ node }) => n.Identifier.check(node) && node.name === name,
];

export const arrowFunctionMatchers = (
    name: string
): Parameters<typeof findAstJs>[1] => [
    ({ node }) => n.VariableDeclaration.check(node),
    ({ node }) =>
...
export const variableDeclaratorMatchers = (
    name: string
): Parameters<typeof findAstJs>[1] => [
    ({ node }) => n.VariableDeclaration.check(node),
    ({ node }) =>
    ...
    if (availableImports.length === 0) return [];
        ...
        const usedImportsLocal = (findAstJs(
            ...
            [
                ({ node }) =>
                    n.Identifier.check(node) &&
                    availableImports.some(([, local]) => local === node.name),
            ],
            node => {
                return n.Identifier.check(node) ? () => node.name : undefined;
            }
        )[0] ?? []) as string[];
        return availableImports.filter(([, local]) =>
            usedImportsLocal.includes(local)
        );
    }
};
...
export const getFunctionOrDeclarationValue = (
    ast: types.namedTypes.Node,
    name: string,
    fallbackFunction: ExpressionKind = b.arrowFunctionExpression(
        [],
        b.blockStatement([])
    )
) => {
    const variableDeclarationValueSearchResults = findAstJs(
        ...
        node =>
            n.VariableDeclarator.check(node) && node.init != undefined
                ? () => node.init
                : undefined
    )[0];
    const functionDeclarationSearchResults = findAstJs(
        ast,
        functionMatchers(name),
        node => (n.FunctionDeclaration.check(node) ? () => node : undefined)
    )[0] as
        | [
    ...
    const variableDeclarationExpression =
        variableDeclarationValueSearchResults &&
        variableDeclarationValueSearchResults.length > 0
            ? (variableDeclarationValueSearchResults[0] as ExpressionKind)
            : undefined;
    const functionDeclaration =
        functionDeclarationSearchResults &&
        functionDeclarationSearchResults.length > 0
            ...
            : undefined;
    return (
        variableDeclarationExpression ?? functionDeclaration ?? fallbackFunction
    );
};
...
export const variableDeclarationAst = (importNames: [string, string][]) =>
    importNames.length > 0
        ? b.variableDeclaration(
              "let",
              importNames.map(([, local]) =>
                  b.variableDeclarator(b.identifier(local))
              )
          )
        : undefined;
...
export const getRootReferenceIndexes = (
    ast: types.namedTypes.Node,
    names: [string, string][]
) =>
    findAstJs(
        ...
        [
            ({ node }) =>
                n.Identifier.check(node) &&
                (names.some(([, local]) => local === node.name) ||
                    node.name === "data"),
        ],
        node =>
            n.Identifier.check(node)
...
const isDeclaredIn = (
    ...
    const propertyCheck = (nd: any) =>
        (n.Property.check(nd) || n.ObjectProperty.check(nd)) &&
        nd.value === node;
    const objectPatternCheck = (node: any) => n.ObjectPattern.check(node);
    if (type === "ObjectPattern") {
        return typeCheckAncestors(
            node,
            meta,
            propertyCheck,
            objectPatternCheck
...
export const findAlias = (
        ...
        enter(node: types.namedTypes.Node | Node) {
            if (result !== undefined || ignore.includes(node)) {
                ...
                ((n.Identifier.check(node.key) &&
                    node.key.name === identifier) ||
                    (n.Identifier.check(node.value) &&
                        node.value.name === identifier))
            ) {
    ...
    return [
        undefined,
        new Error(
            "Couldn't find alias or even the identifier itself"
        ) as FindAliasError,
    ];
};
...
export const findDefinition = (
    ...
    return [
        undefined,
        new Error("Couldn't find definition") as FindDefinitionError,
    ];
};
...
export const identifierIsDeclarable = (
    ...
    try {
        const searchResults = (findAstJs(
            ...
            [
                ({ node }) =>
                    n.Identifier.check(node) && node.name === identifier,
            ],
            node =>
                n.Identifier.check(node)
                    ? meta => {
                          ...
                          if (isDeclaredIn(node, meta, "ObjectPattern"))
                              return false;
                          throw new Error(
                              "Cannot predict indentifier declarability for this ast."
                          );
                      }
                    : undefined
        )[0] ?? [true]) as boolean[];
    ...
    if (n.ObjectPattern.check(node)) {
        ...
        if (n.Identifier.check(node.value)) {
            if (n.Identifier.check(node.key)) {
                if (
                    ...
                    if (n.ObjectPattern.check(ast)) {
                        ...
                        if (
                            n.Identifier.check(foundAlias) &&
                            foundAlias.name === node.key.name
                        )
                            return [[], undefined];
                        ...
                        return [
                            undefined,
                            new Error(
                                "Cant merge Property into ast of different type"
                            ) as MergeNodesError,
                        ];
                    }
                } else {
                    return [
                        undefined,
                        new Error(
                            "Some of the requested identifiers are already in use."
                        ) as MergeNodesError,
                    ];
                }
            } else {
                return [
                    undefined,
                    new Error(
                        "Unsupported type for key of Property."
                    ) as MergeNodesError,
                ];
            }
        } else {
            return [
                undefined,
                new Error(
                    "Unsupported type for value of Property"
                ) as MergeNodesError,
            ];
        }
        ...
        if (node.id != undefined) {
            if (n.Identifier.check(node.id)) {
                ...
                if (
                    ...
                    return [
                        undefined,
                        new Error(
                            "Couldn't find a function to merge into"
                        ) as MergeNodesError,
                    ];
                else if (
                    n.FunctionDeclaration.check(def) ||
                    n.ArrowFunctionExpression.check(def)
                ) {
                        ...
                        if (targetParameter !== undefined) {
                            ...
                            if (mergeResult[0] !== undefined)
                                ...
                                return [
                                    undefined,
                                    new Error(
                                        "Couldn't merge function parameters"
                                    ) as MergeNodesError,
                                ];
                    ...
                    return [
                        undefined,
                        new Error(
                            "Found variable declaration is not of type function"
                        ) as MergeNodesError,
                    ];
                }
            } else {
            ...
            return [
                undefined,
                new Error(
                    "Can't merge anonymous function into an ast"
                ) as MergeNodesError,
            ];
        }
...
export const findDeclarableIdentifier = (
    ast: Node | types.namedTypes.Node,
    identifier: string,
    walker: typeof jsWalk = jsWalk
) => {
...
export const imports = (
    target: Program | types.namedTypes.Program,
    source: string
) => {
    ...
    type AliasesMap = Map<
        string,
        types.namedTypes.Identifier | types.namedTypes.MemberExpression
    >;
    return {
        ...base,
        getAliases: function (
        ...
        add: function (
            ...
            if (
                ...
                const localIdentifier =
                    declarableIdentifier !== exportN
                        ? b.identifier(declarableIdentifier)
                        : undefined;
                ...
                if (exportN === "*" || matchingImports === undefined) {
                    ...
                    const insertionPoint = matchingImports.find(
                        ({ specifiers }) =>
                            specifiers.every(
                                specifier =>
                                    n.ImportSpecifier.check(specifier) ||
                    ...
                    insertionPoint?.specifiers.push(
                        newImportSpecifierAst as ImportSpecifier
                    );
                }
            }
        ...
        removeAll: function () {
            ...
            if (matchingImports !== undefined) {
                for (const declaration of matchingImports) {
                    const indexToRemove = target.body.findIndex(
...
export const svelteRawLiteralReplacements = (
    rawLiteral: string,
    replacement: string
) => {
    ...
    return [
        [
            new RegExp(`=[${whitespaceCharsRegex}]*${rawLiteral}`, "g"),
            `={${replacement}}`,
        ],
        [
            new RegExp(`(?<!(=${whitespaceCharsRegex}*))${rawLiteral}`, "g"),
