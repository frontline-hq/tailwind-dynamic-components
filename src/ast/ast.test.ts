...
describe("getFunctionOrDeclarationValue", () => {
    test("fallback arrow function specified", () => {
        ...
        const resultAst = getFunctionOrDeclarationValue(
            ast.$ast,
            "load",
            fallbackFunction
        );
    ...
    test("function", () => {
        ...
        const ast = parseModule(code);
        const resultAst = getFunctionOrDeclarationValue(ast.$ast, "load");
        const result = generateCode(resultAst);
        expect(result.code).toMatchInlineSnapshot(`
            "function load() {
                console.log(\\"load\\")
            }"
...
describe("identifierIsDeclarable", () => {
    ...
    test("Test identifier in unsupported ast", () => {
        ...
        const ast = parseModule(code);
        expect(identifierIsDeclarable(ast.$ast, "load")[1]).toBeInstanceOf(
            Error
        );
    });
});
...
describe("findAlias", () => {
    describe("object pattern", () => {
        ...
        test("Simple with ...rest property", () => {
            ...
            const result = resultAst ? generateCode(resultAst).code : "";
    ...
    describe("merge function declaration", () => {
        ...
        describe("... into arrow function declaration", () => {
            test("Simple declaration", () => {
                ...
                expect(result).toEqual([[], undefined]);
                expect(generateCode(ast).code).toMatchInlineSnapshot(`
					"import {i} from \\"@inlang/sdk-js\\"
					const hndl = (
					    {
					        key: alias
            ...
            test("Rename parameters", () => {
                ...
                expect(
                    originalName ? generateCode(originalName).code : ""
                ).toBe("alias");
                expect(newName ? generateCode(newName).code : "").toBe(
                    "parameter1.key"
                );
                expect(scope ? generateCode(scope).code : "")
...
describe("imports", () => {
    describe("references", () => {
        test("empty file", () => {
            const code = ``;
            const ast = parseModule(code).$ast;
            expect(imports(ast as Program, "source").findReferences()).toBe(
                undefined
            );
        });
        test("single matching import", () => {
            ...
            expect(result?.[0] ? generateCode(result[0]).code : "")
                .toMatchInlineSnapshot(`
				"import {module} from \\"source\\""
			`);
        });
        test("multiple matching imports", () => {
            ...
            expect(result?.[0] ? generateCode(result[0]).code : "")
				...
				"import {module} from \\"source\\""
			`);
            expect(result?.[1] ? generateCode(result[1]).code : "")
                .toMatchInlineSnapshot(`
				"import {module2} from \\"source\\""
			`);
        });
    ...
    describe("add", () => {
        describe("Regular ...", () => {
            ...
            test("... into regular, duplicate", () => {
				...
				`;
                const ast = parseModule(code).$ast;
                imports(ast as Program, "source").add(
                    "exportN",
                    "aliasDuplicateN"
                );
                expect(generateCode(ast).code).toMatchInlineSnapshot(`
...
`);
    ...
    describe("getAliases", () => {
        test("Detect miscellaneous aliases", () => {
			...
			`;
            const ast = parseModule(code).$ast;
            const { aliases } = imports(ast as Program, "source").getAliases(
                "default",
                "requested"
            );
            ...
            expect(regularAlias ? generateCode(regularAlias).code : "").toEqual(
                "aliasN"
            );
            expect(
                regular2Alias ? generateCode(regular2Alias).code : ""
            ).toEqual("export2N");
            expect(defaultAlias ? generateCode(defaultAlias).code : "").toEqual(
                "defaultAlias2"
            );
            expect(
                namespaceAlias ? generateCode(namespaceAlias).code : ""
            ).toEqual("namespaceAlias");
            expect(
                requestedAlias ? generateCode(requestedAlias).code : ""
            ).toEqual("namespaceAlias.requested");
        });
        test("Return error for nonexistent alias", () => {
			...
			`;
            const ast = parseModule(code).$ast;
            const { error } = imports(ast as Program, "source").getAliases(
                "nonexistent"
            );
...
describe("svelte raw literal replacements", () => {
    test("01", () => {
        const replacements = svelteRawLiteralReplacements(
            "literal",
            "identifier"
        );
    });
